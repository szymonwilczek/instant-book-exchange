...
describe("getMatchingOffers", () => {
    ...
    it("should sort by matchScore then ownerRank if tiers are equal", async () => {
        ...
        // Exact same tier (silver)
        // Book Rank 10: Score 100
        // Book Rank 1: Score 100
        const bookRank10 = {
            _id: "b1", title: "Book A", owner: { _id: "o1", username: "Rank10" }, status: "available"
        };
        const bookRank1 = {
            _id: "b2", title: "Book A", owner: { _id: "o2", username: "Rank1" }, status: "available"
        };
        ...
        const results = await getMatchingOffers("test@example.com");
        // Both score 100. Tiers equal (silver). Sort by rank (asc: lower is better).
        // So Rank 1 (o2) should appear before Rank 10 (o1).
